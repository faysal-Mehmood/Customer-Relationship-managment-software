...
if (isset($_GET['advSearch']) && $_GET['advSearch'] == 'true') {
    if (isset($_POST['firstNameStr'])) {
        ...
        $searchString = " and c.firstName LIKE '%{$post['firstNameStr']}%' and c.lastName LIKE '%{$post['lastNameStr']}%'"
                  ...
                  . " and c.weightage LIKE '%{$post['stateStr']}%' and c.marksobtained LIKE '%{$post['countryStr']}%'"
                  . " and c.totalmarks LIKE '%{$post['zipStr']}%' and c.Phone LIKE '%{$post['phoneStr']}%'"
                  . " and c.subject LIKE '%{$post['secondPhoneStr']}%' and c.startdate LIKE '%{$post['faxStr']}%'"
...
<div class="outer">
  ...
  <div class="leadsMenu">
    ...
    <span class="searchLeads">
      ...
      ?>
      Search For:  <input type="text" style="width:130px"  class="searchText <?php echo $hide; ?>" value="<?php echo $text; ?>">
...
<?php    
    ...
    foreach ($sortOrder as $row => $field) {
        ...
        if($printName=='Name' ||$printName=='FatherName' ||$printName=='marksobtained' ||$printName=='Address'||$printName=='Phone'){
?>
      <th>
...
foreach ($leads as $row => $lead) {
    ...
    foreach ($sortOrder as $row => $field) {
        ...
        else if ($printName =='FatherName'){
            echo "<td>" . html_entity_decode($lead->{"$columnName"}) . "</td>";
            
        }
        else if ($printName =='marksobtained'){
            echo "<td>" . html_entity_decode($lead->{"$columnName"}) . "</td>";
            
        }
        else if ($printName =='Address'){
            echo "<td>" . html_entity_decode($lead->{"$columnName"}) . "</td>";
            
        }
        else if ($printName =='Phone'){
            echo "<td>" . html_entity_decode($lead->{"$columnName"}) . "</td>";
